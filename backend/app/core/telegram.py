...
def validate_telegram_init_data(
    init_data: str, bot_token: Optional[str] = None
    ...
    try:
        ...
        secret_key = hmac.new(
            "WebAppData".encode("utf-8"), bot_token.encode("utf-8"), hashlib.sha256
        ).digest()
...
async def get_telegram_user_profile(telegram_id: int) -> Optional[Dict]:
    """
    Get up-to-date user profile from Telegram Bot API
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Failed to get Telegram user profile: {response.status_code} {response.text}"
                )

        return None
    except Exception as e:
        print(f"Error fetching Telegram user profile: {e}")
        return None
